...
- **The password gate discards the URL it intercepted.** A visitor sent to `/collections/all?click_id=…` lands on `/` after typing the password — attribution gone before the snippet ever ran. On a real store there is no gate; the runner (M4) re-opens the gated URL after unlocking so it observes the funnel, not the gate.
- **Horizon's product cards put the image over the anchor.** Playwright's "element receives events" check refuses the click a human's click still lands (the theme handles it in JS). The runner clicks when it can and otherwise navigates to the card's `href`.
